...
"""Fehler auf P"""
"Skript"
sig_P_skript = np.sqrt(3)/100*(P0-P)/P #Rel Fehler
sig_T_sensor = 0 #absolut
sig_T_konv = 0.027*(T-T0) #rel
...
doc.generate_tex('Tabelle_sys_Fehler_G1')
gerade=-0.045*T+57.43
plt.errorbar(T,lamb/1000, label='Lambda')
plt.errorbar(T,lamb/1000+c, label='Lambda+sig_sys')
plt.errorbar(T,lamb/1000-c, label='Lambda-sig_sys')
plt.title("Maximale Variation der systematischen Fehler Gruppe 1")
plt.xlabel("T")
plt.ylabel("Lambda")
plt.errorbar(T,gerade, label='Literaturwert')
plt.legend(loc=4)
#%%
sig_T_konv = 0.00 #rel
"Daten Gruppe 2"
...
P0=984
P=P_G2
sig_P_skript = np.sqrt(3)/100*(P0-P)/P
#print"sig_lamb_skript= ",(sig_P_skript*R*T).round()/1000
#print"sig_lamb_sensor=", (sig_T_sensor*lamb/T).round()/1000
#print"sig_lamb_konv=", (sig_T_konv*lamb).round()/1000
...
gerade=-0.045*T+57.43
plt.errorbar(T,lamb/1000, label='Lambda')
plt.errorbar(T,lamb/1000+f, label='Lambda+sig_sys')
plt.errorbar(T,lamb/1000-f, label='Lambda-sig_sys')
plt.title("Maximale Variation der systematischen Fehler Gruppe 2")
plt.xlabel("T")
plt.ylabel("Lambda")
plt.errorbar(T,gerade, label='Literaturwert')
plt.legend()"""