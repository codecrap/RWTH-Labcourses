...
"Rohdaten"
x0=22.8*10**(-2)
sig_x0=0.1*10**(-2) #~1mm Ablesefehler in beide Richtungen (geschaetzt)
m=np.array([0.1205,0.1003,0.0503,0.0200])
sig_m=np.array([0.0001,0.0001,0.0001,0.0001])
...
"Plot"
plt.errorbar(-x,m*g,xerr=sig_x ,yerr=sig_m*g, fmt=".")
plt.xlabel('-x')
plt.ylabel('m*g')
plt.title("Bestimmung von D (m*g=-Dx)")

"Lineare Regression und mit in den Plot"
data=p.lineare_regression_xy(-x,m*g,sig_x,sig_m*g)
print data
