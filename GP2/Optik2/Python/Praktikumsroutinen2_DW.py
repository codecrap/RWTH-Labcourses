...
def residuen(x,y,ex,ey,ux,uy,Parameterx,Parametery,save=False,save_folder='plots',resolution=100,k=0,l=0,o=0,p=0,ftsize=20,ca=3,cr=3,mksizea=1,mksizer=1):
    '''
    ...
    if type(ex)==int and ex==0:
         ...
         h='a=('+('{0:5.4f}').format(a*1000000)+r'$ \pm$'+('{0:9.4f}').format(ea*1000000)+r'$)*10^{-6} m$'
         i='b=('+('{0:5.4f}').format(b*1000)+r'$ \pm$'+('{0:9.4f}').format(eb*1000)+r'$)*10^{-3} m$'
         j=r'$\frac{\chi^2}{ndof}=$'+('{0:9.2f}').format(chiq_ndof)
         ...
         ax0.set_ylabel(r" " + Parametery+ uy, fontsize = ftsize)
         ax1.tick_params(labelsize=18)
         ax0.tick_params(labelsize=18)
         ax1.set_xlabel(r" " + Parameterx+ ux, fontsize = ftsize)
    ...
    if save==True:
        ...
        fig1.savefig(save_folder+"/Test.png",dpi=resolution,bbox_inches='tight')
        #dpi ist auflösung
        #bbox_inches='tight'soll nur den Graph ohne weißen Rand nehmen  
