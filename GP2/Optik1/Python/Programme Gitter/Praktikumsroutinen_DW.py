...
def residuen(x,y,ex,ey,ux,uy,Parameterx,Parametery, title = " ",k=0,l=0,o=0,p=0,ftsize=15,ca=3,cr=3,mksizea=1,mksizer=1):
    '''
    ...
    if type(ex)==int and ex==0:
         ...
         h='a='+('({0:9.4f}').format(a*10**6)+'+/-'+('{0:9.4f}) 1/mm').format(ea*10**6)
         ...
         ax0.set_title( title + ": " + Parametery +' gegen '+Parameterx +' (blau) und Ausgleichsgerade (rot)' , fontsize = ftsize)
         ax0.grid()
        ...
        res=y-Ausgleichsgerade
        #hilfsvariablen für Text im Plot
        h='a='+('({0:9.4f}').format(a*10**6)+'+/-'+('{0:9.4f}) 1/mm').format(ea**6)
        i='b='+('{0:9.4f}').format(b)+'+/-'+('{0:9.4f} ').format(eb)
